...
url_tempo_login="https://jira.geniie.net/login.jsp"
url_tempo_jira="https://jira.geniie.net/rest/tempo-timesheets/4/worklogs/"


curl -X POST -L -k -s -c .cookie.tempo \
    -A 'User-Agent: Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1)' \
    --data-urlencode "os_username=daisuke.murata@denso-ten.com" \
    --data-urlencode "os_password=dnten#DTEN" \
    --data-urlencode "login=ログイン" \
    "${url_tempo_login}" 

sleep 2

curl -X POST -k -s -b .cookie.tempo \
    -A 'User-Agent: Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1)' \
    --data-urlencode "comment=あいうえお" \
    --data-urlencode "started=2021-11-23" \
    --data-urlencode "timeSpentSeconds=7200" \
    --data-urlencode "worker=JIRAUSER16417" \
    "${url_tempo_jira}" 
...
#     --data-urlencode "USER=c302890" \
#     --data-urlencode "PASSWORD=dnten#DTEN05" \
...
#     --data-urlencode "TCDR_NTYEAR=2021" \
#     --data-urlencode "TCDR_NTMONTH=10" \
